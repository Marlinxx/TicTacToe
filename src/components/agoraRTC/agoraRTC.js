...
class AgoraRTCIntegration extends Component {
  ...
  muteClickHandler(track) {
    ...
    this.RTMChannel.sendMessage({
      text: msg
    }).then((res) => console.warn('message sent successfully', msg)).catch((err) => console.warn(err));
  }
  ...
  render() {
    const {
      volumeLevel, message, networkQuality, isAudioEnabled, isVideoEnabled, isTutorAudioControlEnabled, isTutorVideoControlEnabled
    } = this.state;
    return (
